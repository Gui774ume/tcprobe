...
func (cc *CgroupContext) UnmarshalBinary(data []byte) (int, error) {
	...
	cc.Name = NullTerminatedString(data[8 : 8+CgroupNameLength])
...
func (pc *ProcessContext) UnmarshalBinary(data []byte) (int, error) {
	...
	pc.Comm = NullTerminatedString(data[cursor : cursor+TaskCommLength])
	cursor += TaskCommLength

	if len(data[cursor:]) < TaskCommLength {
		return 0, fmt.Errorf("while parsing ProcessContext.PPIDComm: got len %d, needed %d: %w", len(data[cursor:]), TaskCommLength, err)
	}
	pc.PPIDComm = NullTerminatedString(data[cursor : cursor+TaskCommLength])
	cursor += TaskCommLength
	...
	pc.PPID = ByteOrder.Uint32(data[cursor+8 : cursor+12])
	// padding
	cursor += 16
